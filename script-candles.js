...
function initializeApp() {
    ...
    injectNavigationMenu('18-candles');
    
    // Initialize all event listeners
    initializeNavigation();
    initializeCandleItems();
...
// ========================================
// CANDLE ITEMS INTERACTION
// ========================================
function initializeCandleItems() {
    const candleItems = document.querySelectorAll('.candle-item');
    
    candleItems.forEach((item, index) => {
        // Add click event
        item.addEventListener('click', () => handleCandleClick(index + 1));
        ...
        item.addEventListener('keypress', (e) => {
            if (e.key === 'Enter' || e.key === ' ') {
                e.preventDefault();
                handleCandleClick(index + 1);
            }
        });
    });
}

function handleCandleClick(candleNumber) {
    console.log(`Candle ${candleNumber} clicked`);
    
    // Add animation effect
    const candleItem = document.querySelector(`.candle-item[data-number="${candleNumber}"]`);
    if (candleItem) {
        candleItem.style.animation = 'none';
        setTimeout(() => {
            candleItem.style.animation = 'fadeInLeft 0.6s ease-out forwards';
        }, 10);
    }
    ...
    if (navigator.share) {
        try {
            await navigator.share({
                title: "18 Candles - Nemy's 18th Birthday",
                text: "Check out the special 18 Candles ceremony for Nemy's 18th Birthday celebration!",
...
function showCustomShareOptions() {
    ...
    const text = encodeURIComponent("Check out the 18 Candles ceremony for Nemy's 18th Birthday!");
...
function trackShareEvent(platform) {
    ...
    //         'event_page': '18-candles'
    //     });
    // }
}

function logPageView() {
    console.log('18 Candles page viewed');
    
    // Example Google Analytics page view (uncomment if GA is implemented)
    // if (typeof gtag !== 'undefined') {
    //     gtag('config', 'GA_MEASUREMENT_ID', {
    //         'page_title': '18 Candles - Nemy',
    //         'page_path': '/18-candles.html'
    //     });
    // }
}
