...
function initializeApp() {
    ...
    injectNavigationMenu('18-blue-bills');
...
function initializeBillItems() {
    ...
    billItems.forEach((item, index) => {
        // Add click event
        item.addEventListener('click', () => handleBillClick(index + 1));
        ...
        item.addEventListener('keypress', (e) => {
            if (e.key === 'Enter' || e.key === ' ') {
                e.preventDefault();
                handleBillClick(index + 1);
            }
        });
    });
}

function handleBillClick(billNumber) {
    console.log(`Bill ${billNumber} clicked`);
    
    // Add animation effect
    const billItem = document.querySelector(`.bill-item[data-number="${billNumber}"]`);
    if (billItem) {
        billItem.style.animation = 'none';
        setTimeout(() => {
            billItem.style.animation = 'fadeInLeft 0.6s ease-out forwards';
        }, 10);
    }
    ...
    if (navigator.share) {
        try {
            await navigator.share({
                title: "18 Blue Bills - Nemy's 18th Birthday",
                text: "Check out the 18 Blue Bills tradition for Nemy's 18th Birthday celebration!",
...
function showCustomShareOptions() {
    ...
    const text = encodeURIComponent("Check out the 18 Blue Bills tradition for Nemy's 18th Birthday!");
...
function trackShareEvent(platform) {
    ...
    //         'event_page': '18-blue-bills'
    //     });
    // }
}

function logPageView() {
    console.log('18 Blue Bills page viewed');
    
    // Example Google Analytics page view (uncomment if GA is implemented)
    // if (typeof gtag !== 'undefined') {
    //     gtag('config', 'GA_MEASUREMENT_ID', {
    //         'page_title': '18 Blue Bills - Nemy',
    //         'page_path': '/18-blue-bills.html'
    //     });
    // }
}
