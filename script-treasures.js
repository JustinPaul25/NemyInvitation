...
function initializeApp() {
    ...
    injectNavigationMenu('18-treasures');
    
    // Initialize all event listeners
    initializeNavigation();
    initializeTreasureItems();
...
function handleBackClick() {
    ...
    setTimeout(() => {
        window.location.href = '18-shots.html';
    }, 500);
}
...
// ========================================
// TREASURE ITEMS INTERACTION
// ========================================
function initializeTreasureItems() {
    const treasureItems = document.querySelectorAll('.treasure-item');
    
    treasureItems.forEach((item, index) => {
        // Add click event
        item.addEventListener('click', () => handleTreasureClick(index + 1));
        ...
        item.addEventListener('keypress', (e) => {
            if (e.key === 'Enter' || e.key === ' ') {
                e.preventDefault();
                handleTreasureClick(index + 1);
            }
        });
    });
}

function handleTreasureClick(treasureNumber) {
    console.log(`Treasure ${treasureNumber} clicked`);
    
    // Add animation effect
    const treasureItem = document.querySelector(`.treasure-item[data-number="${treasureNumber}"]`);
    if (treasureItem) {
        treasureItem.style.animation = 'none';
        setTimeout(() => {
            treasureItem.style.animation = 'fadeInLeft 0.6s ease-out forwards';
        }, 10);
    }
    ...
    if (navigator.share) {
        try {
            await navigator.share({
                title: "18 Treasures - Nemy's 18th Birthday",
                text: "Check out the 18 Treasures ceremony for Nemy's 18th Birthday celebration!",
...
function showCustomShareOptions() {
    ...
    const text = encodeURIComponent("Check out the 18 Treasures ceremony for Nemy's 18th Birthday!");
...
function trackShareEvent(platform) {
    ...
    //         'event_page': '18-treasures'
    //     });
    // }
}

function logPageView() {
    console.log('18 Treasures page viewed');
    
    // Example Google Analytics page view (uncomment if GA is implemented)
    // if (typeof gtag !== 'undefined') {
    //     gtag('config', 'GA_MEASUREMENT_ID', {
    //         'page_title': '18 Treasures - Nemy',
    //         'page_path': '/18-treasures.html'
    //     });
    // }
}
