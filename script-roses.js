...
// ========================================
// ROSE ITEMS INTERACTION
// ========================================
function initializeRoseItems() {
    const roseItems = document.querySelectorAll('.rose-item');
    
    roseItems.forEach((item, index) => {
        // Add click event
        item.addEventListener('click', () => handleRoseClick(index + 1));
        ...
        item.addEventListener('keypress', (e) => {
            if (e.key === 'Enter' || e.key === ' ') {
                e.preventDefault();
                handleRoseClick(index + 1);
            }
        });
    });
}

function handleRoseClick(roseNumber) {
    console.log(`Rose ${roseNumber} clicked`);
    
    // Add animation effect
    const roseItem = document.querySelector(`.rose-item[data-number="${roseNumber}"]`);
    if (roseItem) {
        roseItem.style.animation = 'none';
        setTimeout(() => {
            roseItem.style.animation = 'fadeInLeft 0.6s ease-out forwards';
        }, 10);
    }
    ...
    if (navigator.share) {
        try {
            await navigator.share({
                title: "18 Roses - Nemy's 18th Birthday",
                text: "Check out the special 18 Roses ceremony for Nemy's 18th Birthday celebration!",
...
function showCustomShareOptions() {
    ...
    const text = encodeURIComponent("Check out the 18 Roses ceremony for Nemy's 18th Birthday!");
...
function showNotification(message) {
    ...
    notification.style.cssText = `
        ...
        background: rgba(212, 87, 125, 0.95);
        color: white;
        padding: 1rem 2rem;
        border-radius: 50px;
...
function logPageView() {
    console.log('18 Roses page viewed');
    
    // Example Google Analytics page view (uncomment if GA is implemented)
    // if (typeof gtag !== 'undefined') {
    //     gtag('config', 'GA_MEASUREMENT_ID', {
    //         'page_title': '18 Roses - Nemy',
    //         'page_path': '/18-roses.html'
    //     });
    // }
}
